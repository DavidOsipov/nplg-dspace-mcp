...
from scripts.pyright_identity import PyrightIdentityError
...
@dataclass(frozen=True, slots=True)
class _CandidateBindingFixture:
    commit: str
    tree: str


@dataclass(frozen=True, slots=True)
class _ArtifactEvidenceFixture:
    path: str


@dataclass(frozen=True, slots=True)
class _BoundArtifactFixture:
    evidence: _ArtifactEvidenceFixture
    identity: tuple[int, ...] = ()
...
class _PassingRunner:
    def __call__(self, command: GateCommand) -> ChildResult:
        stdout_by_name = {
            ...
            "ruff": b"[]",
            "test-gate": _test_gate_success_bytes(),
        }
...
def _test_gate_success_bytes() -> bytes:
    policy = cast(
        "dict[str, JsonValue]",
        load_json_value(
            (PROJECT_ROOT / "security" / "coverage-policy.json").read_bytes()
        ),
    )
    modules = cast("list[JsonValue]", policy["decision_modules"])
    return canonical_json_bytes(
        {
            "decision_module_branch_percent": [
                [cast("str", path), 100.0] for path in modules
            ],
            "diff_branch_arc_percent": 95.25,
            "diff_line_percent": 96.5,
            "project_branch_percent": 95.75,
        }
    )
...
def _local_gates_request_fixture(tmp_path: Path) -> release_module.LocalGatesRequest:
    worktree = (tmp_path / "worktree").resolve()
    worktree.mkdir()
    _ = _git(worktree, "init", "-q")
    _ = (worktree / "source.txt").write_bytes(b"source\n")
    _ = _git(worktree, "add", "source.txt")
    _ = _git(
        worktree,
        "-c",
    ...
    candidate = _git(worktree, "rev-parse", "HEAD").decode("ascii").strip()
    candidate_tree = _git(worktree, "rev-parse", "HEAD^{tree}").decode("ascii").strip()
    artifacts = (tmp_path / "artifacts").resolve()
    artifacts.mkdir()
    wheel = artifacts / "nplg_mcp-1.0.0-py3-none-any.whl"
    sdist = artifacts / "nplg_mcp-1.0.0.tar.gz"
    _ = wheel.write_bytes(b"wheel-bytes")
    _ = sdist.write_bytes(b"sdist-bytes")
    return release_module.LocalGatesRequest(
        worktree=worktree,
        output=(tmp_path / "local-gates").resolve(),
...
def test_isolated_tool_driver_allows_only_the_proved_in_tree_purelib() -> None:
    request = release_module.LocalGatesRequest(
        ...
        candidate="1" * 40,
        candidate_tree="2" * 40,
        wheel=Path("/external/package.whl"),
        sdist=Path("/external/package.tar.gz"),
    )
    command = release_module.local_gate_commands(
        request,
        SUBJECT_DIGEST,
...
def test_gate_battery_accepts_exact_test_gate_success_contract(tmp_path: Path) -> None:
    result = run_gate_battery(
        (_command(tmp_path, "test-gate"),),
        _PayloadRunner(_test_gate_success_bytes(), b""),
        monotonic=_Clock(),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "missing-field",
            _test_gate_success_bytes().replace(
                b',"project_branch_percent":95.75', b"", 1
            ),
            b"",
        ),
        (
            "extra-field",
            _test_gate_success_bytes().replace(b"{", b'{"extra":true,', 1),
            b"",
        ),
        (
            "wrong-type",
            _test_gate_success_bytes().replace(b"95.75", b'"95.75"', 1),
            b"",
        ),
        (
            "integer-type",
            _test_gate_success_bytes().replace(b"95.75", b"95", 1),
            b"",
        ),
        (
            "boolean-type",
            _test_gate_success_bytes().replace(b"95.75", b"true", 1),
            b"",
        ),
        (
            "nonfinite",
            _test_gate_success_bytes().replace(b"95.75", b"NaN", 1),
            b"",
        ),
        (
            "below-range",
            _test_gate_success_bytes().replace(b"95.75", b"-0.01", 1),
            b"",
        ),
        (
            "above-range",
            _test_gate_success_bytes().replace(b"95.75", b"100.01", 1),
            b"",
        ),
        (
            "duplicate-decision-module",
            _test_gate_success_bytes().replace(
                b'[["src/nplg_mcp/admission.py",100.0]',
                (
                    b'[["src/nplg_mcp/admission.py",100.0],'
                    b'["src/nplg_mcp/admission.py",100.0]'
                ),
        ...
        (
            ...
            _test_gate_success_bytes().replace(
                b'{"decision_module_branch_percent":',
                b"".join(
                    (
                        b'{"project_branch_percent":95.75,',
                        b'"decision_module_branch_percent":',
                    )
                ),
                1,
        ...
        ("stderr", _test_gate_success_bytes(), b"unexpected warning\n"),
    ],
)
def test_gate_battery_rejects_invalid_test_gate_success_contract(
    tmp_path: Path,
    case: str,
    stdout: bytes,
    stderr: bytes,
...
def test_local_gates_records_external_requirements_as_not_passed(
    ...
    request = _local_gates_request_fixture(tmp_path)
    ...
    assert report.candidate_commit == request.candidate
    assert report.candidate_tree == request.candidate_tree
    assert tuple(item.kind for item in report.artifacts) == (
        "python-wheel",
        "python-sdist",
    )
    assert tuple(item.path for item in report.artifacts) == (
        request.wheel.as_posix(),
        request.sdist.as_posix(),
    )
    assert tuple(item.size for item in report.artifacts) == (11, 11)
    assert tuple(item.sha256 for item in report.artifacts) == (
    ...
    assert (request.output / "release-gate-results.json").is_file()


def test_local_gates_rejects_non_git_worktree(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    worktree = (tmp_path / "not-git").resolve()
    worktree.mkdir()
    artifacts = (tmp_path / "artifacts").resolve()
    artifacts.mkdir()
    wheel = artifacts / "package.whl"
    sdist = artifacts / "package.tar.gz"
    ...
    with pytest.raises(ReleaseVerificationError, match="Git candidate"):
...
@pytest.mark.parametrize(
    ("case", "field", "value"),
    [
        ("wrong-candidate", "candidate", "0" * 40),
        ("wrong-tree", "candidate_tree", "0" * 40),
        ("short-candidate", "candidate", "1" * 39),
        ("uppercase-tree", "candidate_tree", "A" * 40),
    ],
)
def test_local_gates_rejects_wrong_or_malformed_candidate_binding(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    case: str,
    field: str,
    ...
    request = _local_gates_request_fixture(tmp_path)
    request = (
        replace(request, candidate=value)
        if field == "candidate"
        else replace(request, candidate_tree=value)
    )
    monkeypatch.setattr(
        release_module,
        "_validate_pyright_runtime_identity",
...
def test_local_gates_ignores_git_replacement_objects(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    replacement_input = request.worktree / "replacement-only.txt"
    _ = replacement_input.write_bytes(b"replacement tree\n")
    _ = _git(request.worktree, "add", replacement_input.name)
    replacement_tree = _git(request.worktree, "write-tree").decode("ascii").strip()
    replacement_commit = (
        _git(
            request.worktree,
            "-c",
    ...
    _ = _git(request.worktree, "replace", request.candidate, replacement_commit)
    request = replace(request, candidate_tree=replacement_tree)
    monkeypatch.setattr(
        release_module,
        "_validate_pyright_runtime_identity",
        _stable_runtime_identity,
...
def test_local_gates_rejects_annotated_tag_object_at_head(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    ...
    tag_object = (
        _git(request.worktree, "rev-parse", "refs/tags/candidate-tag")
        .decode("ascii")
        .strip()
    )
    _ = (request.worktree / ".git" / "HEAD").write_text(
        "ref: refs/tags/candidate-tag\n",
        encoding="ascii",
    )
    request = replace(request, candidate=tag_object)
    monkeypatch.setattr(
        release_module,
        "_validate_pyright_runtime_identity",
        _stable_runtime_identity,
...
def test_local_gates_rejects_non_commit_object_at_head(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    blob = (
        _git(request.worktree, "hash-object", "-w", "source.txt")
        .decode("ascii")
        .strip()
    )
    _ = (request.worktree / ".git" / "HEAD").write_text(
    ...
    request = replace(request, candidate=blob)
    monkeypatch.setattr(
        release_module,
        "_validate_pyright_runtime_identity",
        _stable_runtime_identity,
...
def test_local_gates_rejects_dirty_submodule_hidden_by_local_ignore(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    child_source = (tmp_path / "child-source").resolve()
    child_source.mkdir()
    _ = _git(child_source, "init", "-q")
    _ = (child_source / "child.txt").write_bytes(b"clean\n")
    _ = _git(child_source, "add", "child.txt")
    _ = _git(
        child_source,
        "-c",
        "user.name=Release Gate Test",
    ...
    _ = _git(
        ...
        "add",
        "-q",
        child_source.as_posix(),
        "child",
    )
    _ = _git(request.worktree, "add", ".gitmodules", "child")
    _ = _git(
        request.worktree,
        "-c",
        "user.name=Release Gate Test",
    ...
    candidate = _git(request.worktree, "rev-parse", "HEAD").decode("ascii").strip()
    candidate_tree = (
        _git(request.worktree, "rev-parse", "HEAD^{tree}").decode("ascii").strip()
    )
    request = replace(
        request,
        candidate=candidate,
    ...
    _ = _git(request.worktree, "config", "submodule.child.ignore", "all")
    _ = (request.worktree / "child" / "child.txt").write_bytes(b"dirty\n")
    monkeypatch.setattr(
        release_module,
        "_validate_pyright_runtime_identity",
        _stable_runtime_identity,
    ...
    with pytest.raises(ReleaseVerificationError, match="clean Git candidate"):
        _ = release_module.run_local_gates(
            request,
            runner=_PassingRunner(),
            monotonic=_Clock(),
...
def test_local_gates_rejects_dirty_candidate(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    _ = (request.worktree / "dirty.txt").write_bytes(b"dirty")
    monkeypatch.setattr(
        release_module,
        "_validate_pyright_runtime_identity",
        _stable_runtime_identity,
...
@pytest.mark.parametrize(
    "case",
    ["missing", "swapped", "symlink", "special", "inside-worktree"],
)
def test_local_gates_rejects_unsafe_or_mistyped_artifacts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    case: str,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    if case == "missing":
        request.wheel.unlink()
    elif case == "swapped":
        request = replace(request, wheel=request.sdist, sdist=request.wheel)
    elif case == "symlink":
        request.wheel.unlink()
        request.wheel.symlink_to(request.sdist)
    elif case == "special":
        request.wheel.unlink()
        os.mkfifo(request.wheel)
    else:
        inside = request.worktree / ".git" / "package.whl"
        _ = inside.write_bytes(b"inside")
        request = replace(request, wheel=inside)
    monkeypatch.setattr(
        release_module,
        "_validate_pyright_runtime_identity",
        _stable_runtime_identity,
    ...
    with pytest.raises(ReleaseVerificationError, match="artifact"):
        _ = release_module.run_local_gates(
            request,
            runner=_PassingRunner(),
            monotonic=_Clock(),
...
def test_local_gates_rejects_malformed_git_object_output(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    payload: bytes,
) -> None:
    ...
    class MalformedGit:
        def run(
            ...
            *,
            ...
            del cwd, environment
            return CommandResult(argv=argv, returncode=0, stdout=payload, stderr=b"")

    monkeypatch.setattr(release_module, "SystemGit", MalformedGit)

    with pytest.raises(ReleaseVerificationError, match="Git candidate HEAD"):
        _ = release_module.run_local_gates(
            request,
            runner=_PassingRunner(),
            monotonic=_Clock(),
...
def test_local_gates_rejects_relative_artifact_path(tmp_path: Path) -> None:
    request = replace(
        _local_gates_request_fixture(tmp_path),
        wheel=Path("relative-package.whl"),
    )

    with pytest.raises(
        ReleaseVerificationError, match="artifact path must be absolute"
    ):
        _ = release_module.run_local_gates(
            request,
            runner=_PassingRunner(),
...
def test_local_gates_wraps_artifact_hashing_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)

    def unavailable_artifact_hash(path: Path, *, max_bytes: int) -> str:
        del path, max_bytes
        error_message = "injected hashing failure"
        raise PyrightIdentityError(error_message)

    monkeypatch.setattr(
        release_module,
        "stable_regular_file_sha256",
        unavailable_artifact_hash,
    )

    with pytest.raises(
        ReleaseVerificationError, match="artifact identity is unavailable"
    ):
        _ = release_module.run_local_gates(
            request,
            runner=_PassingRunner(),
...
def test_local_gates_rejects_artifact_mutation_during_identity_capture(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)

    def mutating_artifact_hash(path: Path, *, max_bytes: int) -> str:
        del max_bytes
        _ = path.write_bytes(b"mutated-during-hash")
        return "sha256:" + hashlib.sha256(b"mutated-during-hash").hexdigest()

    monkeypatch.setattr(
        release_module,
        "stable_regular_file_sha256",
        mutating_artifact_hash,
    )

    with pytest.raises(ReleaseVerificationError, match=r"changed while.*captured"):
        _ = release_module.run_local_gates(
            request,
            runner=_PassingRunner(),
            monotonic=_Clock(),
...
def test_local_gates_rejects_duplicate_artifact_capture_fault(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    duplicate = _BoundArtifactFixture(
        evidence=_ArtifactEvidenceFixture(path=request.wheel.as_posix())
    )

    def duplicate_artifact(*args: object, **kwargs: object) -> _BoundArtifactFixture:
        del args, kwargs
        return duplicate
    ...
    with pytest.raises(
        ReleaseVerificationError, match="exactly one wheel and one sdist"
    ):
        _ = release_module.run_local_gates(
            request,
            runner=_PassingRunner(),
...
def test_local_gates_rejects_unavailable_or_noncanonical_worktree(
    tmp_path: Path,
    case: str,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    ...
    if case == "symlink":
        worktree.symlink_to(request.worktree, target_is_directory=True)
        expected = "canonical directory"
    request = replace(request, worktree=worktree)
...
def test_local_gates_rejects_candidate_binding_stability_fault(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)
    bindings = iter(
        (
            _CandidateBindingFixture(request.candidate, request.candidate_tree),
            _CandidateBindingFixture("0" * 40, request.candidate_tree),
        )
    )

    def drifting_binding(
        candidate_request: release_module.LocalGatesRequest,
        *,
        root: Path,
    ) -> _CandidateBindingFixture:
        del candidate_request, root
        return next(bindings)

    monkeypatch.setattr(
        release_module,
        "_capture_candidate_binding",
        drifting_binding,
    )
    monkeypatch.setattr(
        release_module,
    ...
    with pytest.raises(ReleaseVerificationError, match="Git candidate changed"):
        _ = release_module.run_local_gates(
            request,
            runner=_PassingRunner(),
            monotonic=_Clock(),
...
def test_local_gates_rejects_artifact_change_during_run(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request = _local_gates_request_fixture(tmp_path)

    class MutatingArtifactRunner:
        mutated = False

        def __call__(self, command: GateCommand) -> ChildResult:
            if not self.mutated:
                _ = request.wheel.write_bytes(b"changed-wheel")
                self.mutated = True
            return _PassingRunner()(command)
    ...
    with pytest.raises(ReleaseVerificationError, match=r"artifact.*changed"):
        _ = release_module.run_local_gates(
            request,
            runner=MutatingArtifactRunner(),
            monotonic=_Clock(),
        )
...
def test_local_gate_commands_bind_python_tools_to_the_isolated_driver() -> None:
    ...
    test_gate = next(command for command in commands if command.name == "test-gate")
    assert test_gate.argv[3:] == (
        "--worktree",
        ...
        "origin/main",
        "--output-dir",
        "/external/release-evidence/test-gate",
    )
...
def test_local_gates_reject_source_drift(
    ...
    request = _local_gates_request_fixture(tmp_path)
    source = request.worktree / "source.txt"
...
def test_local_gates_cli_dispatches_success(
    ...
    requests: list[release_module.LocalGatesRequest] = []
    report = release_module.LocalGatesReport(
        local_status="passed",
        candidate_commit="1" * 40,
        candidate_tree="2" * 40,
        artifacts=(
            release_module.LocalArtifactEvidence(
                kind="python-wheel",
                path=(tmp_path / "dist" / "package.whl").as_posix(),
                size=1,
                sha256="sha256:" + "3" * 64,
            ),
            release_module.LocalArtifactEvidence(
                kind="python-sdist",
                path=(tmp_path / "dist" / "package.tar.gz").as_posix(),
                size=1,
                sha256="sha256:" + "4" * 64,
            ),
        ),
    ...
    assert (
        release_module.main(
            [
                ...
                "2" * 40,
                "--wheel",
                (tmp_path / "dist" / "package.whl").as_posix(),
                "--sdist",
                (tmp_path / "dist" / "package.tar.gz").as_posix(),
            ]
        )
        == 0
    )
    ...
    assert requests[0].candidate == "1" * 40
    assert requests[0].candidate_tree == "2" * 40
    assert requests[0].wheel == tmp_path / "dist" / "package.whl"
    assert requests[0].sdist == tmp_path / "dist" / "package.tar.gz"
